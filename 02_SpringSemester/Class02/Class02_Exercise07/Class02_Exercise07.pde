...
void mesh(float posX, float posY, float scayl, PImage colorData) {
  ...
  for (int i=0; i<colorData.width-1; i++) { //<---- REMOVE ONE! (…-1)
    ...
    for (int j=0; j<colorData.height; j++) {
      ...
      float thisZval = map(saturation(thisPix), 0, 255, 0, 100);

      //THAT : The NEXT row
      color thatPix = colorData.get(i+1, j);
      float thatZval = map(saturation(thatPix), 0, 255, 0, 100);

      fill(thisPix);
      vertex(i*scayl, j*scayl, thisZval);
      vertex((i+1)*scayl, j*scayl, thatZval);
    }
    endShape();
  }
